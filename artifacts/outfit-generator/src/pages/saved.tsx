...
import { searchWardrobe } from "@/lib/searchWardrobe";
...
export default function SavedPage() {
  ...
  // Scroll to top on first keystroke of a new search
  useEffect(() => {
    if (searchQuery && !prevQueryRef.current) {
      // Scroll parent scroll container (the <main> element) to the top
  ...
  const searchResults = trimmedQuery
    ? searchWardrobe(trimmedQuery, allItems, outfits ?? [])
    : null;
    ...
    <div ref={pageRef} className="min-h-full flex flex-col pt-8 px-4 pb-8 bg-secondary/10 relative">
      ...
      <div className="mb-5 relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-9 py-2.5 rounded-full border-2 border-black bg-white
        ...
        <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-2">
                {searchResults.items.map((item) => (
                  ...
                  >
                    ...
                    {item.isFavorite && <span className="ml-auto text-sm flex-shrink-0">❤️</span>}
                  </button>
                ))}
              </div>
            </section>
                  ...
                  >
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      ...
                      {(outfit.items ?? []).length === 0 && (
                        <div className="w-10 h-10 border border-black/20 rounded bg-muted" />
                      )}
                    </div>
                    <div className="min-w-0">
          ...
          {searchResults.items.length === 0 && searchResults.groups.length === 0 && (
            <div className="flex flex-col items-center justify-center text-center py-16">
              ...
              <p className="font-display font-bold text-lg uppercase tracking-tight">Nothing found</p>
